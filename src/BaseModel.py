...
class BaseModel:
    """Base model implementing common observing functionality
    """
    ...
    def full_mask(self, data=None):
        if data is None:
            data = self._observations

        return data.isnull().values == False
    
    
    def country_mask(self, data=None):
        return self.full_mask(data)[:, -1]
    
    
    def anwser_mask(self, data=None):
        return self.full_mask(data)[:, :-1]
